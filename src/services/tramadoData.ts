...
export const formatReceptionNumber = (number: number) =>
  `REC-${String(number).padStart(6, '0')}`;
...
export const linkCustomerVehicle = async (customerId: string, vehicleId: string) => {
  ...
  const { error } = await client
    ...
    }, { onConflict: 'user_id,customer_id,vehicle_id', ignoreDuplicates: true });
...
export const createReception = async (input: CreateReceptionInput): Promise<ReceptionRecord> => {
  ...
  try {
    await recordMileage(input.vehicleId, mileage, `Recepción ${formatReceptionNumber(data.reception_number)}`);
  } catch (mileageError) {
    console.warn('La recepción se guardó, pero no se pudo registrar el kilometraje histórico.', mileageError);
  }

  return data as ReceptionRecord;
};

export const upsertReceptionInspection = async (
  input: UpsertInspectionInput
): Promise<ReceptionInspectionRecord> => {
  const client = getClient();
  const user = await getCurrentUser();

  const { data: reception, error: receptionError } = await client
    .from('workshop_receptions')
    .select('id')
...
export const uploadReceptionPhotos = async (
  receptionId: string,
  files: File[],
  photoType: ReceptionPhotoRecord['photo_type'] = 'entry'
): Promise<ReceptionPhotoRecord[]> => {
  ...
  for (const file of files) {
    ...
    if (error) {
      await client.storage.from('reception-photos').remove([path]);
      throw error;
    }
...
export const listRecentReceptions = async (limit = 20): Promise<ReceptionRecord[]> => {
  const client = getClient();
  const user = await getCurrentUser();
  const { data, error } = await client
    .from('workshop_receptions')
